...
public class VersusScreenAnalyserImpl implements VersusScreenAnalyser {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(VersusScreenAnalyserImpl.class);
    ...
    public Game analyse(BufferedImage versusImage) throws OCRException {
        String playerOneGamertag = getGamerTag(versusImage, Side.PLAYER_ONE, PLAYER_ONE_X, PLAYER_Y, PLAYER_WIDTH, PLAYER_HEIGHT);
        String playerTwoGamertag = getGamerTag(versusImage, Side.PLAYER_TWO, PLAYER_TWO_X, PLAYER_Y, PLAYER_WIDTH, PLAYER_HEIGHT);
    ...
    Game analyse(BufferedImage versusImage, String playerOneGamertag, String playerTwoGamertag) throws OCRException {
        Umvc3Character firstCharacterTeamOne = getCharacter(versusImage, Side.PLAYER_ONE, 1, CHARACTER_1_PLAYER_ONE_X, CHARACTER_1_Y, CHARACTER_WIDTH, CHARACTER_HEIGHT);
        Umvc3Character secondCharacterTeamOne = getCharacter(versusImage, Side.PLAYER_ONE, 2, CHARACTER_2_PLAYER_ONE_X, CHARACTER_2_Y, CHARACTER_WIDTH, CHARACTER_HEIGHT);
        Umvc3Character thirdCharacterTeamOne = getCharacter(versusImage, Side.PLAYER_ONE, 3, CHARACTER_3_PLAYER_ONE_X, CHARACTER_3_Y, CHARACTER_WIDTH, CHARACTER_HEIGHT);
        if (firstCharacterTeamOne == secondCharacterTeamOne || firstCharacterTeamOne == thirdCharacterTeamOne || secondCharacterTeamOne == thirdCharacterTeamOne) {
            throw new OCRException(String.format("OCR recognised the following team one: %s, %s, %s, but it\'s impossible to have the same character on a team multiple times.", firstCharacterTeamOne, secondCharacterTeamOne, thirdCharacterTeamOne));
        }
        Umvc3Character firstCharacterTeamTwo = getCharacter(versusImage, Side.PLAYER_TWO, 1, CHARACTER_1_PLAYER_TWO_X, CHARACTER_1_Y, CHARACTER_WIDTH, CHARACTER_HEIGHT);
        Umvc3Character secondCharacterTeamTwo = getCharacter(versusImage, Side.PLAYER_TWO, 2, CHARACTER_2_PLAYER_TWO_X, CHARACTER_2_Y, CHARACTER_WIDTH, CHARACTER_HEIGHT);
        Umvc3Character thirdCharacterTeamTwo = getCharacter(versusImage, Side.PLAYER_TWO, 3, CHARACTER_3_PLAYER_TWO_X, CHARACTER_3_Y, CHARACTER_WIDTH, CHARACTER_HEIGHT);
        if (firstCharacterTeamTwo == secondCharacterTeamTwo || firstCharacterTeamTwo == thirdCharacterTeamTwo || secondCharacterTeamTwo == thirdCharacterTeamTwo) {
            throw new OCRException(String.format("OCR recognised the following team two: %s, %s, %s, but it\'s impossible to have the same character on a team multiple times.", firstCharacterTeamTwo, secondCharacterTeamTwo, thirdCharacterTeamTwo));
        }
        AssistType firstAssistTeamOne = getAssistType(new Color(getRGB(versusImage, BACKGROUND_PLAYER_ONE_CHARACTER_ONE_X, BACKGROUND_Y)));
        AssistType secondAssistTeamOne = getAssistType(new Color(getRGB(versusImage, BACKGROUND_PLAYER_ONE_CHARACTER_TWO_X, BACKGROUND_Y)));
        AssistType thirdAssistTeamOne = getAssistType(new Color(getRGB(versusImage, BACKGROUND_PLAYER_ONE_CHARACTER_THREE_X, BACKGROUND_Y)));
        AssistType firstAssistTeamTwo = getAssistType(new Color(getRGB(versusImage, BACKGROUND_PLAYER_TWO_CHARACTER_ONE_X, BACKGROUND_Y)));
        AssistType secondAssistTeamTwo = getAssistType(new Color(getRGB(versusImage, BACKGROUND_PLAYER_TWO_CHARACTER_TWO_X, BACKGROUND_Y)));
        AssistType thirdAssistTeamTwo = getAssistType(new Color(getRGB(versusImage, BACKGROUND_PLAYER_TWO_CHARACTER_THREE_X, BACKGROUND_Y)));
        Player playerOne = new Player(playerOneGamertag);
        Player playerTwo = new Player(playerTwoGamertag);
        Team teamOne = new Team(firstCharacterTeamOne, firstAssistTeamOne, secondCharacterTeamOne, secondAssistTeamOne, thirdCharacterTeamOne, thirdAssistTeamOne);
        Team teamTwo = new Team(firstCharacterTeamTwo, firstAssistTeamTwo, secondCharacterTeamTwo, secondAssistTeamTwo, thirdCharacterTeamTwo, thirdAssistTeamTwo);
     ...
     * in case OCR fails
     */
    private Umvc3Character getCharacter(BufferedImage versusImage, Side side, int characterNumber, int x, int y, int w, int h) throws OCRException {
        BufferedImage image = getSubImage(versusImage, x, y, w, h);
    ...
    private boolean equalsWithinMargin(Color left, Color right, int margin) {
        return equalsWithinMargin(left.getRed(), right.getRed(), margin) && equalsWithinMargin(left.getGreen(), right.getGreen(), margin) && equalsWithinMargin(left.getBlue(), right.getBlue(), margin);
    }
    ...
    public boolean canBeVersusScreen(BufferedImage image) {
        return checkBlackPixel(image, 200, 60) && checkBlackPixel(image, 1080, 60) && checkBlackPixel(image, 200, 680) && checkBlackPixel(image, 1080, 680);
    }
    ...
    private BufferedImage getSubImage(BufferedImage image, int x, int y, int w, int h) {
        return image.getSubimage(x * image.getWidth() / SCREEN_WIDTH, y * image.getHeight() / SCREEN_HEIGHT, w * image.getWidth() / SCREEN_WIDTH, h * image.getHeight() / SCREEN_HEIGHT);
    }
    
    @java.beans.ConstructorProperties({"engine"})
    @java.lang.SuppressWarnings("all")
    public VersusScreenAnalyserImpl(final OCREngine engine) {
        this.engine = engine;
    }
}